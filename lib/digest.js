...
digest = {
    ...
    get: function() {
        return digest.generate()
            .then(function() {
                return Promise.resolve('Ciao! Here\'s your digest ' + config.hosts.client + 'digest');
            });
    },

    generate: function(mode) {
        if (mode !== 'scheduled') twillio.send({ body: 'One second as I generate your digest ( ﾟ▽ﾟ)/' });
        return Promise.join(
            digest._getProductHunt(),
            digest._getHackerNews(),
            digest._getDesignerNews(),
            digest._getMedium(),
            function(productHunt, hackerNews, designerNews, medium) {
                var digestDoc = { articles: [] };
                digestDoc.articles = digestDoc.articles.concat(productHunt, hackerNews, designerNews, medium);
